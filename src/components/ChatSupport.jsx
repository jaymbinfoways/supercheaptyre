...
const ChatSupport = () => {
    ...
    if (isMinimized) {
        return (
            <button
                ...
                className='fixed bottom-4 right-4 w-14 h-14 rounded-full bg-[#FF0000] shadow-lg z-50 grid place-items-center focus:outline-none focus:ring-2 focus:ring-white/70'
            >
    ...
    return (
        <div className='fixed bottom-4 right-4 bg-[#FF0000] border-2 border-[#FF0000] rounded-xl max-w-72 w-72 p-0 shadow-lg z-50 overflow-hidden'>

            <CgCompressRight
                aria-label='Minimize chat'
                onClick={() => setIsMinimized(true)}
                className='w-6 h-6 rounded-full bg-white/20 absolute top-2 right-2 hover:bg-white/30 text-white grid place-items-center cursor-pointer'
            >
            </CgCompressRight>
